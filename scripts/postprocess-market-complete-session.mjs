...
const singlePageAsset='doing-market-single-page-registration.js';
const singlePageSrc=path.join(root,singlePageAsset);
if(!fs.existsSync(singlePageSrc))throw new Error(`missing ${singlePageAsset}`);
fs.copyFileSync(singlePageSrc,path.join(out,singlePageAsset));
...
const singlePageTag='<script src="/doing-market-single-page-registration.js?v=20260826-1"></script>';
if(!publicHtml.includes(topicTag))publicHtml=publicHtml.replace('</body>',`${topicTag}\n</body>`);
if(!publicHtml.includes(singlePageTag))publicHtml=publicHtml.replace('</body>',`${singlePageTag}\n</body>`);
if(!publicHtml.includes(topicAsset))throw new Error('Market topic filter enhancer missing: /market/public/');
if(!publicHtml.includes(singlePageAsset))throw new Error('Market single-page registration enhancer missing: /market/public/');
fs.writeFileSync(publicFile,publicHtml,'utf8');
console.log(JSON.stringify({result:'PASS',asset,targets:['/market/','/market/session/'],publicAssets:[topicAsset,singlePageAsset],publicTarget:'/market/public/',legacyDialogSelectorPreserved:true,legacySessionNameSelectorPreserved:true,asyncSessionHydration:true,singlePageRegistration:true},null,2));