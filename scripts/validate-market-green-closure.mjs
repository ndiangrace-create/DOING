...
const topicJs=fs.readFileSync('doing-market-topic-filter.js','utf8');
...
for(const t of ['market-topic-chips','data-topic-id','sortSessions','eventId','openRegistration','aspect-ratio:1/1','object-fit:contain'])assert.ok(topicJs.includes(t),'主題分類／全場次排序能力缺少：'+t);
...
const builtTopicJs=fs.readFileSync('.doing-2-site/doing-market-topic-filter.js','utf8');
...
for(const t of ['frontBootstrap','openRegistration','market-2bl-front','doing-market-public-2bl.js','doing-market-topic-filter.js','bottom-nav'])assert.ok(builtPublic.includes(t),'正式 /market/public/ 能力遺失：'+t);
...
assert.ok(builtTopicJs.includes('market-topic-chips')&&builtTopicJs.includes('sortSessions'),'主題必須是小型分類按鈕且全場次依日期排序');
