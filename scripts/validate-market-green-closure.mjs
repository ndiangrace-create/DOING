...
for(const bad of ['doing-market-session-settings-v16','doing-market-role-ui-v17','doing-market-public-query'])assert.ok(!build.includes(`'${bad}.css'`)&&!build.includes(`'${bad}.js'`),'正式建置不得再發布舊 Market 補丁：'+bad);
...
console.log(JSON.stringify({result:'PASS',public:'discovery→register',member:'history→refund request→reread',admin:'session create/update + metric→list→write→reread + refund suggestion→confirm→reread',onsite:'checkin→reread',responsive:'desktop+mobile',legacyMarketPatches:false,dbChanges:0},null,2));
