...
async function directAdminLoginJourney(browser,viewport){
  ...
  await page.goto(`${BASE}/market/`);await page.waitForSelector('#marketLineLogin');assert.match(await page.locator('.mk-login-card').innerText(),/不需要重新申請/);
  await page.goto(`${BASE}/market/?member_token=member-test-token`);await page.waitForURL(/\/market\/\?tenant=demo&admin_token=/,{timeout:10000});await page.waitForSelector('#sessionList .mk-card');assert.equal(await page.locator('a[href="/apply/"]').count(),0);assertNoErrors();await page.close();
}
async function adminJourney(browser,viewport){
  ...
  await page.goto(`${BASE}/market/session/?tenant=demo&admin_token=${encodeURIComponent(adminToken)}&sessionId=S1`);await page.waitForSelector('#kpis .mk-stat');assert.equal(await page.locator('.mk-session-tabs button').count(),8);
...
async function publicJourney(browser,viewport){
  ...
  await page.evaluate(()=>localStorage.setItem('doing_member_token','member-test-token'));await page.reload();await page.waitForLoadState('domcontentloaded');
