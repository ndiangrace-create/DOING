...
for(const text of ['報名活動','我的紀錄','線上客服','我要申請 DOING'])assert.ok(home.includes(text),'首頁缺少一般民眾入口：'+text);
...
assert.ok(css.includes('@media(max-width:760px)')&&css.includes('@media(max-width:430px)'),'首頁缺少手機響應斷點');
assert.ok(css.includes('font-size:clamp(20px,2.3vw,30px)'),'主按鈕沒有老花友善字級');
...
assert.ok(build.includes("const fp=path.join(out,'doing-2.html')")&&build.includes('doing-2-home-v11.css'),'v11 首頁樣式沒有只注入根首頁');
assert.ok(!/for\(const file of \['market-center\.html','market-session\.html'\]\)[\s\S]*doing-2-home-v11/.test(build),'首頁樣式污染 Market');
...
console.log(JSON.stringify({
  ...
  simulatedJourneys:[
    '首頁 → 搜尋 → 公開活動',
    '首頁 → 報名活動',
    '首頁 → 我的紀錄 → 單一 LINE 登入回跳',
    '首頁 → 線上客服 → 快捷問題／自由輸入',
    '首頁 → 我要申請 → 申請流程無客服 gate'
  ],
  responsive:['desktop','tablet','mobile'],
  buttonStyle:'3d-rounded-rectangle',
  productionWrites:0
},null,2));
