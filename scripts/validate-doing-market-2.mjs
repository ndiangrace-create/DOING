...
const allowedMarketStorage=new Set(['doing_market_token','doing_market_tenant']);for(const [name,source] of [['center',html],['session',session]]){const keys=[...source.matchAll(/setStore\('([^']+)'/g)].map(m=>m[1]);assert.ok(keys.every(k=>allowedMarketStorage.has(k)),`${name} 不得用 localStorage/sessionStorage 保存正式營運資料：${keys.filter(k=>!allowedMarketStorage.has(k)).join(',')}`);for(const badKey of ['registration','payment','refund','amount','fee','deposit','seat','capacity','limit','status'])assert.ok(!new RegExp(`localStorage\\.setItem\\([^)]*${badKey}`,'i').test(source),`${name} 不得把 ${badKey} 營運資料寫 localStorage`)}
