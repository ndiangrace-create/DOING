...
for(const bad of ['2bl-v7','DOING-Market-App','doingmarket://']){for(const [name,source] of [['public',publicPage],['center',html],['session',session]])assert.ok(!source.includes(bad),`${name} 不得耦合：${bad}`)}
for(const source of [html,session])assert.ok(!source.includes('localStorage.setItem'),'主辦 Market 頁不得把正式營運資料寫 localStorage');
for(const [name,source] of [['market-public.html',publicPage],['market-center.html',html],['market-session.html',session]]){
