...
const copyFiles=[
  ...
  'doing-2-shell.css','doing-2-shell.js','doing-2-home-v11.css','doing-2-home-v11.js','doing-market-2bl.css',
...
for(const [rel,targetFile] of Object.entries(routeMap)){
  const dir=path.dirname(path.join(out,rel));fs.mkdirSync(dir,{recursive:true});
  const depth=rel.split('/').length-1,prefix='../'.repeat(depth);
  fs.writeFileSync(path.join(out,rel),redirect(prefix+targetFile));
}

fs.writeFileSync(path.join(out,'_headers'),`/*\n  Cache-Control: no-store\n  X-Content-Type-Options: nosniff\n  Referrer-Policy: strict-origin-when-cross-origin\n`);
console.log(JSON.stringify({result:'PASS',output:'.doing-2-site',root:'doing-2.html',routes:Object.keys(routeMap),shell:shellTargets,homeV11:true,marketCompact:['market-center.html','market-session.html'],memberDirectReturn:true,applicationFlow:true,applicationFixedProducts:'v5',workspaceMarketRouter:true},null,2));