...
const homeCss=fs.readFileSync('doing-home-refresh.css','utf8');
...
assert(homeRefresh.includes("applicationMode=target==='apply'")&&homeRefresh.includes("content.querySelector('#startDoingApplication')?.click()")&&!homeRefresh.includes("content.querySelector('#startDoingSupport')?.click()"),'營運申請仍混入客服模式');
...
assert(page.includes('class="doing-helper-frame-head"')&&page.includes('class="doing-helper-stage"'),'智慧申請缺少單一大型框架');
const frameHeadAt=page.indexOf('class="doing-helper-frame-head"'),applicationButtonAt=page.indexOf('id="startDoingApplication"'),stageAt=page.indexOf('class="doing-helper-stage"'),formAt=page.indexOf('id="signupForm"');
assert(frameHeadAt<applicationButtonAt&&applicationButtonAt<stageAt&&stageAt<formAt,'智慧申請未形成單一申請框架');
...
assert(homeRefresh.includes("function openPublicSupport()")&&homeRefresh.includes("loadOrganizerDetails('apply',true)"),'客服未獨立開啟，或申請導引按鈕未前往智慧申請');
assert(page.includes('跟著 DOING 智慧小幫手完成申請'),'缺少智慧申請頁標題');
assert(page.includes('id="startDoingApplication"')&&!page.includes('id="startDoingSupport"')&&!page.includes('id="doingHelperWelcome"'),'營運帳號申請仍出現客服模式或客服對話框');
