...
for(const token of ["'/apply/':'smart-application.html'","'/workspace/':'workspace.html'","'/me/':'member-panel.html'","'/market/':'market-center.html'","'doing-smart-activation-v5.js'","'doing-formal-application-bridge.js'","'doing-auto-activation-status.js'","'doing-global-entry.js'","'doing-application-completion.js'","'doing-workspace-product-router.js'"]) assert.ok(build.includes(token),'Cloudflare Pages 缺少短網址／必要功能：'+token);
for(const removed of ["'/admin/'","'/onsite/'","'/platform/'","'/operations/'"]) assert.ok(!build.includes(removed+':'),'不應再發布獨立路由：'+removed);

console.log(JSON.stringify({result:'PASS',journey:['/apply/ 單頁','工作類型＋使用方式＋基本資料','LINE member 驗證','既有空間直接回 /me/','新申請 DB 自動開通','/me/','/workspace/','/market/'],helperRole:'customer_service_only',applicationGate:false,singlePage:true,duplicateApplicationBlocked:true,duplicateLogin:false,homeBounce:false,newTables:0,workerSchemaChanges:0},null,2));
