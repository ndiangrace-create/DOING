...
assert.ok(visual.features.some(feature=>feature.status==='later'&&feature.label.includes('逐頁套用')),'尚未逐頁套用前不得標示完成');

console.log(JSON.stringify({result:'PASS',standard:'DOING Homepage Pastel v1',paletteColors:Object.keys(palette).length,smallTiles:'rounded-square-only',worldTreeStatus:'standard_done_pages_later'},null,2));
