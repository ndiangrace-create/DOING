...
assert(homeScript.includes('id="doingPublicSupportFab"')&&homeScript.includes('function openPublicSupport()'),'首頁客服必須進入獨立的 DOING 民眾智慧小幫手');
...
assert(homeScript.includes('<h1><span>找活動、報名、預約，</span><span>進度都在 DOING。</span></h1>'),'首頁主標題必須使用兩個完整語意行，禁止拆開「預約」');
assert(!homeScript.includes('<h1>找活動、報名、預約，<br>'),'首頁主標題不得用可造成單字破碎的 br 舊版排法');
assert(homeScript.includes('class="doing-public-section doing-slash-scenes"')&&homeScript.includes('平常是設計師、跑現場監工，假日又是手作老師。')&&homeScript.includes('辦市集，也辦活動、手作與體驗')&&homeScript.includes('美甲老師要服務客人，也會開課'),'首頁缺少直接可見的多角色斜槓人生情境');
assert(!homeScript.includes('<button type="button" data-public-support-open>報名／預約客服</button>'),'頁尾不得重複出現造成奇怪方框的客服按鈕');
