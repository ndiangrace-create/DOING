...
assert(shell.includes("setProperty('position',desktop?'fixed'"),'桌機首頁頂部列不是 position: fixed');
assert(shell.includes("setProperty('top',desktop?'0'"),'桌機首頁頂部列沒有固定 top:0');
assert(shell.includes("setProperty('z-index',desktop?'1000'"),'桌機首頁頂部列層級不足');
assert(shell.includes("setProperty('padding-top',`${top.offsetHeight}px`,'important')"),'固定頂部列未補內容位移，可能遮住首頁');
assert(!shell.includes("if(isPublicHome){document.body?.classList.add('d2-public-home');return;}"),'首頁仍在建立固定列前直接 return');
console.log(JSON.stringify({result:'PASS',applicationGate:'closed',helperRole:'customer-service-only',desktopTopbar:'position-fixed-top-0',mobileTopbar:'hidden<=620px',contentOffset:'topbar-height',unifiedTheme:'all-formal-pages',logoSlot:'reserved',productionWrites:0},null,2));
