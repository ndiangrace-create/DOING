...
const memberAuth={token:'',profile:null,complete:false,resume:null,provider:'',linkedProviders:[],roles:[],applications:[],workspaces:[]};
...
async function initMemberAuth(){
  ...
  const loginError=u.searchParams.get('member_login_error');if(loginError){u.searchParams.delete('member_login_error');history.replaceState({},'',u.pathname+u.search+u.hash);setTimeout(()=>{showMemberView(memberAuth.token?'profile':'login');const x=ensureMemberGate().querySelector('.doing-member-error');if(x)x.textContent=loginError==='identity_resolution_required'?'LINE 登入已成功。這組 Email 或手機可能已有 DOING 帳號，申請已保留，請先同步原 Google 帳號；無法使用舊登入時請聯絡平台協助。':loginError==='email_link_requires_existing_login'?'此 Email 已由 LINE 會員使用，請先以 LINE 登入；系統不會只憑未驗證 Email 自動合併帳號。':loginError==='line_email_permission_required'?'LINE Email 權限尚未完成，請先完成 LINE Login 設定。':loginError==='line_only'?'目前會員端只開放 LINE 登入。':'登入未完成，請重新嘗試。'},20)}
