...
let homepageBootReleased=false;
function releaseHomepageBoot(){if(homepageBootReleased)return;homepageBootReleased=true;document.documentElement.classList.remove('doing-global-boot');document.body?.classList.remove('doing-global-boot')}
...
async function openMemberWorkspaceAdmin(space,calendar=true){const token=incomingToken||(()=>{try{return localStorage.getItem('doing_member_token')||''}catch(_){return''}})();const tenantId=String(space?.id||space?.tenantId||space?.tenant_id||'').trim().toLowerCase();if(!token||!tenantId)return false;const session=await createWorkspaceSession(token,tenantId);const u=workspaceTarget(session.tenantId||tenantId,session.adminToken||'');if(calendar)u.hash='calendar';location.replace(u.toString());return true}
async function handoffToWorkspace(token,status){if(!token)return false;try{localStorage.setItem('doing_member_token',token)}catch(_){}const profile=await getProfile(token);if(!profile.complete){location.replace(memberTarget(token,'account',status||'profile_required').toString());return true}const spaces=Array.isArray(profile.workspaces)?profile.workspaces:[];if(spaces.length===1){await openMemberWorkspaceAdmin(spaces[0],true);return true}if(spaces.length>1){location.replace(memberTarget(token,'operations',status||'ready').toString());return true}location.replace(applicationTarget().toString());return true}
