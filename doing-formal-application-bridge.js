...
function statusCard(title,text,button,onclick,secondary){
  ...
  form.innerHTML=`<div id="d2a"><style>#d2a{display:grid;gap:10px;color:#24343a}#d2a .h,#d2a .c{background:#fff;border:1px solid #dce8e9;border-radius:14px;padding:14px}#d2a .h{background:#eef7f8}#d2a h2{margin:2px 0 5px;font-size:25px}#d2a p{margin:0;color:#687680;line-height:1.6;font-size:16px}.b{min-height:46px;border:0;border-radius:9px;padding:9px 14px;font-size:16px;font-weight:900}.pri{background:#4f8f9d;color:#fff}.alt{background:#fff;border:1px solid #dce8e9;margin-left:8px}</style><section class="h"><small>DOING 營運帳號</small><h2>${title}</h2><p>${text}</p></section><section class="c"><button class="b pri" id="formalPrimary" type="button">${button}</button>${secondary?'<button class="b alt" id="formalSecondary" type="button">重新填寫</button>':''}</section></div>`;
...
async function resumeAfterLine(){
  ...
  statusCard('LINE 驗證完成','正在建立你的 DOING 工作空間，請勿重複送出。','正在建立…',()=>{});
...
async function existingWorkspacePreflight(){
  ...
  try{const r=await originalFetch(API+'?action=getPlatformMemberProfile&member_token='+encodeURIComponent(token),{cache:'no-store'}),d=await r.json().catch(()=>({}));if(!r.ok||d.error||d.ok===false)return;const data=d.data??d.result??d,spaces=Array.isArray(data.workspaces)?data.workspaces:[];if(!spaces.length)return;statusCard('你的工作空間已經開通','這個會員已經有自己的 DOING 工作空間，不需要重新申請。','進入我的 DOING',()=>location.replace(memberOperations().toString()))}catch(_){}
}
