...
const keep=(path)=>{const u=new URL(path,location.origin);for(const k of ['tenant']){const v=p.get(k);if(v)u.searchParams.set(k,v)}return u.toString()};
...
<section class="d2-home-hero" aria-label="DOING 首頁">
  ...
  <nav class="d2-home-account-actions" aria-label="帳號入口">
    <a class="d2-home-account apply" href="/apply/">申請 DOING</a>
    <a class="d2-home-account login" id="d2HomeLogin" href="/me/">登入</a>
  </nav>
</section>
...
try{if(localStorage.getItem('doing_member_token'))$('d2HomeLogin').textContent='我的 DOING'}catch(_){}
...
function registrationUrl(x){const u=new URL('/register/',location.origin);if(x.tenantId)u.searchParams.set('tenant',x.tenantId);if(x.sessionId||x.id)u.searchParams.set('session',x.sessionId||x.id);return u.toString()}
...
function quick(q){const s=q.replace(/\s/g,'');if(/第一次.*報名|怎麼報名|怎麼預約/.test(s))return'先從首頁的「報名活動」找到想參加的場次，點「立即報名」後照畫面完成資料。';if(/進度|紀錄|我的報名/.test(s))return'請先從首頁按「登入」。登入後所有報名、品牌與營運空間都集中在「我的 DOING」同一頁。';if(/付款|取消|退款|改期/.test(s))return'登入「我的 DOING」後打開我的報名，再依該筆活動提供的付款或異動操作處理。';if(/卡住|錯誤|不能|無法/.test(s))return'把「你在哪一頁、按了哪個按鈕、畫面出現什麼提示」告訴我，我會先幫你判斷下一步。';return''}
