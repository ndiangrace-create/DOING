...
async function tenantBillingSnapshot(env,T){
  ...
  const activeBooking=await activeBookingEntitlement(env,T);return {ok:true,policy,paymentProfile,support:{supportEmail:support.supportEmail||'',officialLineUrl:support.officialLineUrl||''},platformCredit:await platformCreditBalance(env,T),booking:activeBooking?{active:true,periodStart:activeBooking.period_start,periodEnd:activeBooking.period_end}:{active:false},charges,summary:{outstanding:charges.reduce((n,x)=>n+Math.max(0,safeNum(x.outstanding)),0),reported:charges.reduce((n,x)=>n+Math.max(0,safeNum(x.reportedAmount)),0)}}
}
