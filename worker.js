...
const TENANT_ROLE_ACTIONS = {
  ...
  settings: new Set(['saveCompanySettings','saveSiteConfig','saveEmailTemplate','testEmail','savePaymentSettings','savePaymentProfile','disablePaymentProfile','saveAgreementTemplate','saveAgreementTemplates','saveVenueMap','applyVenueMap','deleteVenueMap','savePhotoActivity','deletePhotoActivity','savePhotoActivityFrame','deletePhotoActivityFrame','savePromotionRule','deletePromotionRule','saveTenantDomain','checkTenantDomain','saveMarketingAutomation']),
  finance: new Set(['financeOverview','financeReport','adminFinanceAnomalies','getFinance','getPayments','getFinancePaymentGroups','getSessionCashbook','saveFinanceItem','deleteFinanceItem','saveSessionCashItem','deleteSessionCashItem','confirmPayment','confirmRefund','confirmForceRefund','refundDeposit','applyForceRefund','applyForceRefundFM','markPaymentScreenshot','sendPaymentReminder','getInvoiceList','updateInvoiceStatus','downloadSession','saveMembershipPlan','issueMembershipPlan']),
  sessions: new Set(['getDashboard','adminBusinessOverview','getSessionDashboard','getAdminSessionsDashboard','getAdminSessionDashboard','getEventsAdmin','getSessionsAdmin','getOperationUnitsAdmin','getBookingCalendarAdmin','getAvailabilityAdmin','getPromotionRulesAdmin','getOperationalCloseout','createEvent','updateEvent','deleteEvent','createSession','updateSession','copySession','deleteSession','toggleSession','toggleSessionStatus','saveOperationUnit','deleteOperationUnit','saveAvailabilityRule','saveAvailabilityException','saveAnnouncement','deleteAnnouncement','sendNotify','resendInvite','resendRegConfirm','saveConsignmentPeriod','reviewConsignmentApplication','saveWaitlistPolicy']),
  ...
  onsite: new Set(['onsiteSessions','onsiteRegs','onsitePasscodeList','onsitePasscodeGenerate','onsitePasscodeToggle','onsiteShiftList','onsiteShiftStart','onsiteShiftEnd','onsiteMark','checkin','markClear','updateServiceVisit']),
  pos: new Set(['recordPosSale']),
...
const TENANT_ROLE_ALLOW = {
  ...
  pos:['organizer_owner','organizer_admin','session_admin','finance_admin','onsite_staff','platform_super_admin'],
...
// ── v21 正式營運收尾：攤商業績、寄賣 POS、方案、到店、自動候補、網域與行銷 ──
async function verifiedMemberRegistration(env,token,registrationId,tenantId=''){
  const verified=await verifiedPlatformMember(env,token);if(!verified||!verified.row)return null;
  const mid=String(verified.row.id||''),rid=String(registrationId||'').trim();if(!rid)return null;
  let q=`id=eq.${encodeURIComponent(rid)}&select=*`;if(tenantId)q=`tenant_id=eq.${encodeURIComponent(tenantId)}&${q}`;
  const rows=await dbGet(env,'registrations',q).catch(()=>[]),r=rows[0];if(!r)return null;
  if(String(r.platform_member_id||'')===mid||String(r.submitted_by_member_id||'')===mid)return {verified,registration:r};
...
async function hGetMyOperationalTasks(env,p){
  ...
  let linked=[];if(linkedIds.length)linked=await dbGet(env,'registrations',`id=in.(${linkedIds.map(x=>encodeURIComponent(x)).join(',')})&select=id,tenant_id,session_id,operation_unit_id,brand_id,name,brand_name,deposit,review_status,payment_status,created_at`).catch(()=>[]);
  const regs=[...new Map([...own,...linked].map(x=>[String(x.id),x])).values()],ids=regs.map(x=>x.id),reports=ids.length?await dbGet(env,'vendor_sales_reports',`registration_id=in.(${ids.map(x=>encodeURIComponent(x)).join(',')})&select=*`).catch(()=>[]):[],visits=ids.length?await dbGet(env,'service_visits',`registration_id=in.(${ids.map(x=>encodeURIComponent(x)).join(',')})&select=*`).catch(()=>[]):[];
  return jsonOk({registrations:regs,salesReports:reports,serviceVisits:visits,email});
}
async function hSubmitVendorSalesReport(env,b){
  const T=b._tenantId,access=await verifiedMemberRegistration(env,b.member_token||b.memberToken||b.token,b.registrationId,T);if(!access)return jsonErr('只有這筆報名的本人或已授權出攤者可回報');
  const r=access.registration,amount=Number(b.grossSales),date=String(b.reportDate||new Date().toLocaleDateString('sv-SE',{timeZone:'Asia/Taipei'})).slice(0,10);if(!Number.isFinite(amount)||amount<0)return jsonErr('請自行輸入正確的當日營業額');
  const row={id:genId('VSR'),tenant_id:T,session_id:r.session_id,registration_id:r.id,platform_member_id:access.verified.row.id,report_date:date,gross_sales:amount,note:String(b.note||'').trim().slice(0,500),deposit_refund_status:safeNum(r.deposit)>0?'eligible':'not_applicable',submitted_at:nowIso(),updated_at:nowIso()};
  try{await dbInsert(env,'vendor_sales_reports',row)}catch(e){if(String(e.message||'').includes('duplicate'))return jsonErr('這一天已完成回報；正式金額不可覆寫，如需處理請聯絡平台');throw e}
  if(safeNum(r.deposit)>0)await recordNotification(env,{tenantId:T,sessionId:r.session_id,registrationId:r.id,email:platformContactEmail(access.verified.row),eventKey:'deposit_refund_eligible',title:'保證金已進入可退還流程',body:'營業額已完成回報，主辦可依正式退款流程處理保證金。'});
  return jsonOk({ok:true,report:row});
}
async function hGetOperationalCloseout(env,p){
  const T=p._tenantId;if(!await verifyStaff(env,p.email,p.token,T))return jsonErr('無權限');
  const tables=['tenant_domains','vendor_sales_reports','consignment_periods','consignment_applications','consignment_products','pos_sales','pos_sale_items','inventory_movements','membership_plans','membership_subscriptions','service_visits','marketing_automations','marketing_automation_runs'];const out={};
  for(const table of tables)out[table]=await dbGet(env,table,`tenant_id=eq.${encodeURIComponent(T)}&select=*&order=created_at.desc&limit=500`).catch(()=>[]);
  const sessions=await dbGet(env,'sessions',`tenant_id=eq.${encodeURIComponent(T)}&select=id,name,limit_count,current_count,modules_json,status,dates_json&order=created_at.desc`).catch(()=>[]),regs=await dbGet(env,'registrations',`tenant_id=eq.${encodeURIComponent(T)}&select=id,session_id,operation_unit_id,name,brand_name,email,review_status,payment_status,payment_due_at,deposit,created_at&order=created_at.desc&limit=1000`).catch(()=>[]);
  return jsonOk({...out,sessions,registrations:regs});
}
async function hSaveTenantDomain(env,b){
  const T=b._tenantId;if(!await verifyStaff(env,b.email,b.token,T))return jsonErr('無權限');let domain=String(b.domain||'').trim().toLowerCase().replace(/^https?:\/\//,'').replace(/\/.*$/,'').replace(/:\d+$/,'');if(!/^(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z]{2,63}$/.test(domain))return jsonErr('請輸入正確的自有網域');
  const old=await dbGet(env,'tenant_domains',`tenant_id=eq.${encodeURIComponent(T)}&domain=eq.${encodeURIComponent(domain)}&select=*`).catch(()=>[]),id=String(old[0]?.id||genId('DOM')),token=String(old[0]?.verification_token||crypto.randomUUID().replace(/-/g,''));
  if(old.length)await dbUpdate(env,'tenant_domains',`tenant_id=eq.${encodeURIComponent(T)}&id=eq.${encodeURIComponent(id)}`,{status:'active',verification_token:token,requested_by:b.email||'',updated_at:nowIso()});else await dbInsert(env,'tenant_domains',{id,tenant_id:T,domain,is_primary:false,verified:false,status:'active',verification_token:token,requested_by:b.email||'',created_at:nowIso(),updated_at:nowIso()});
  return jsonOk({ok:true,id,domain,verificationToken:token,verificationUrl:`https://${domain}/.well-known/doing-verification.txt`});
}
async function hCheckTenantDomain(env,b){
  const T=b._tenantId;if(!await verifyStaff(env,b.email,b.token,T))return jsonErr('無權限');const id=String(b.id||''),rows=await dbGet(env,'tenant_domains',`tenant_id=eq.${encodeURIComponent(T)}&id=eq.${encodeURIComponent(id)}&select=*`).catch(()=>[]),d=rows[0];if(!d)return jsonErr('找不到網域申請');
  let ok=false,err='';try{const r=await fetch(`https://${d.domain}/.well-known/doing-verification.txt`,{redirect:'follow'}),text=await r.text();ok=r.ok&&text.trim()===String(d.verification_token||'');if(!ok)err='驗證檔內容尚未符合'}catch(e){err='目前無法讀取驗證檔'}
  await dbUpdate(env,'tenant_domains',`tenant_id=eq.${encodeURIComponent(T)}&id=eq.${encodeURIComponent(id)}`,{verified:ok,verified_at:ok?nowIso():null,last_checked_at:nowIso(),verification_error:ok?null:err,updated_at:nowIso()});if(ok)await dbUpdate(env,'tenants',`id=eq.${encodeURIComponent(T)}`,{site_url:`https://${d.domain}`,updated_at:nowIso()});return jsonOk({ok,verified:ok,error:err});
}
async function hSaveConsignmentPeriod(env,b){
  const T=b._tenantId;if(!await verifyStaff(env,b.email,b.token,T))return jsonErr('無權限');const name=String(b.name||'').trim(),startsAt=b.startsAt,endsAt=b.endsAt;if(!name||!startsAt||!endsAt||new Date(endsAt)<=new Date(startsAt))return jsonErr('請填完整且正確的寄賣檔期');const id=String(b.id||genId('CSP')),data={operation_unit_id:String(b.operationUnitId||'')||null,name,starts_at:startsAt,ends_at:endsAt,commission_percent:Math.max(0,Math.min(100,safeNum(b.commissionPercent))),status:['draft','open','closed','settled'].includes(String(b.status))?String(b.status):'draft',config_json:b.config||{},created_by:b.email||'',updated_at:nowIso()},old=await dbGet(env,'consignment_periods',`tenant_id=eq.${encodeURIComponent(T)}&id=eq.${encodeURIComponent(id)}&select=id`).catch(()=>[]);if(old.length)await dbUpdate(env,'consignment_periods',`tenant_id=eq.${encodeURIComponent(T)}&id=eq.${encodeURIComponent(id)}`,data);else await dbInsert(env,'consignment_periods',{id,tenant_id:T,created_at:nowIso(),...data});return jsonOk({ok:true,id});
}
async function hGetConsignmentPeriodsPublic(env,p){const T=p._tenantId;return jsonOk(await dbGet(env,'consignment_periods',`tenant_id=eq.${encodeURIComponent(T)}&status=eq.open&ends_at=gt.${encodeURIComponent(nowIso())}&select=id,name,starts_at,ends_at,commission_percent,config_json&order=starts_at.asc`).catch(()=>[]))}
async function hGetMyConsignment(env,p){
  const T=p._tenantId,verified=await verifiedPlatformMember(env,p.member_token||p.memberToken||p.token);if(!verified||!verified.row)return jsonErr('請先登入 DOING 會員');
  const mid=String(verified.row.id||''),applications=await dbGet(env,'consignment_applications',`tenant_id=eq.${encodeURIComponent(T)}&platform_member_id=eq.${encodeURIComponent(mid)}&select=*&order=created_at.desc`).catch(()=>[]),ids=applications.map(x=>x.id).filter(Boolean);
  const products=ids.length?await dbGet(env,'consignment_products',`tenant_id=eq.${encodeURIComponent(T)}&application_id=in.(${ids.map(x=>encodeURIComponent(x)).join(',')})&select=*&order=created_at.desc`).catch(()=>[]):[];
  return jsonOk({applications,products});
}
async function hApplyConsignment(env,b){
  const T=b._tenantId,verified=await verifiedPlatformMember(env,b.member_token||b.memberToken||b.token);if(!verified)return jsonErr('請先登入 DOING 會員');const periodId=String(b.periodId||''),periods=await dbGet(env,'consignment_periods',`tenant_id=eq.${encodeURIComponent(T)}&id=eq.${encodeURIComponent(periodId)}&status=eq.open&select=id`).catch(()=>[]);if(!periods.length)return jsonErr('寄賣檔期目前未開放');const mid=String(verified.row.id),email=platformContactEmail(verified.row),id=genId('CSA');
  try{await dbInsert(env,'consignment_applications',{id,tenant_id:T,period_id:periodId,platform_member_id:mid,brand_id:String(b.brandId||'')||null,applicant_email:email,brand_name:String(b.brandName||verified.row.name||'').trim(),status:'pending',note:String(b.note||'').trim().slice(0,500),created_at:nowIso(),updated_at:nowIso()})}catch(e){if(String(e.message||'').includes('duplicate'))return jsonErr('你已申請這個寄賣檔期');throw e}return jsonOk({ok:true,id});
}
async function hReviewConsignmentApplication(env,b){const T=b._tenantId;if(!await verifyStaff(env,b.email,b.token,T))return jsonErr('無權限');const id=String(b.id||''),status=['approved','rejected','settled'].includes(String(b.status))?String(b.status):'';if(!id||!status)return jsonErr('審核資料不完整');await dbUpdate(env,'consignment_applications',`tenant_id=eq.${encodeURIComponent(T)}&id=eq.${encodeURIComponent(id)}`,{status,reviewed_by:b.email||'',reviewed_at:nowIso(),updated_at:nowIso()});return jsonOk({ok:true})}
async function hSaveConsignmentProduct(env,b){
  const T=b._tenantId,appId=String(b.applicationId||''),apps=await dbGet(env,'consignment_applications',`tenant_id=eq.${encodeURIComponent(T)}&id=eq.${encodeURIComponent(appId)}&status=eq.approved&select=*`).catch(()=>[]),app=apps[0];if(!app)return jsonErr('寄賣申請尚未通過');let by=String(b.email||''),allowed=await verifyStaff(env,b.email,b.token,T);if(!allowed){const verified=await verifiedPlatformMember(env,b.member_token||b.memberToken||b.token);allowed=verified&&String(verified.row.id)===String(app.platform_member_id);by=verified?platformContactEmail(verified.row):''}if(!allowed)return jsonErr('無權限');
  const name=String(b.name||'').trim(),sku=String(b.sku||'').trim(),stock=Math.max(0,parseInt(b.stock||0,10)||0),price=Math.max(0,safeNum(b.unitPrice));if(!name||!sku)return jsonErr('請填商品名稱與貨號');const id=String(b.id||genId('CPD')),old=await dbGet(env,'consignment_products',`tenant_id=eq.${encodeURIComponent(T)}&id=eq.${encodeURIComponent(id)}&select=*`).catch(()=>[]),data={period_id:app.period_id,application_id:app.id,name,sku,barcode:String(b.barcode||'').trim()||null,unit_price:price,status:'active',config_json:{updatedBy:by},updated_at:nowIso()};if(old.length)await dbUpdate(env,'consignment_products',`tenant_id=eq.${encodeURIComponent(T)}&id=eq.${encodeURIComponent(id)}`,data);else{await dbInsert(env,'consignment_products',{id,tenant_id:T,opening_stock:stock,current_stock:stock,created_at:nowIso(),...data});if(stock>0)await dbInsert(env,'inventory_movements',{id:genId('INV'),tenant_id:T,product_id:id,sale_id:null,movement_type:'opening',quantity_delta:stock,stock_after:stock,note:'期初庫存',operator_email:by,created_at:nowIso()})}return jsonOk({ok:true,id});
}
async function hRecordPosSale(env,b){const T=b._tenantId;if(!await verifyStaff(env,b.email,b.token,T,'finance')&&!await verifyStaff(env,b.email,b.token,T,'onsite'))return jsonErr('無權限');const items=Array.isArray(b.items)?b.items:[],key=String(b.idempotencyKey||'').trim();if(!key||!items.length)return jsonErr('請選擇商品');try{return jsonOk(await dbRpc(env,'record_consignment_pos_sale',{p_tenant_id:T,p_period_id:String(b.periodId||''),p_operator_email:b.email||'',p_payment_method:String(b.paymentMethod||'other'),p_idempotency_key:key,p_items:items}))}catch(e){return jsonErr(String(e.message||'銷售失敗').replace(/^DB RPC [^:]+:\s*/,''))}}
async function hSaveMembershipPlan(env,b){const T=b._tenantId;if(!await verifyStaff(env,b.email,b.token,T,'finance'))return jsonErr('無權限');const name=String(b.name||'').trim(),type=['bundle','visit_pass','membership'].includes(String(b.planType))?String(b.planType):'';if(!name||!type)return jsonErr('方案資料不完整');const id=String(b.id||genId('MPL')),data={operation_unit_id:String(b.operationUnitId||'')||null,service_item_id:String(b.serviceItemId||'')||null,name,plan_type:type,included_quantity:Math.max(0,safeNum(b.includedQuantity)),valid_days:b.validDays?Math.max(1,parseInt(b.validDays,10)):null,price:Math.max(0,safeNum(b.price)),sharing_mode:String(b.sharingMode)==='named_group'?'named_group':'owner_only',status:['draft','active','archived'].includes(String(b.status))?String(b.status):'active',config_json:b.config||{},created_by:b.email||'',updated_at:nowIso()},old=await dbGet(env,'membership_plans',`tenant_id=eq.${encodeURIComponent(T)}&id=eq.${encodeURIComponent(id)}&select=id`).catch(()=>[]);if(old.length)await dbUpdate(env,'membership_plans',`tenant_id=eq.${encodeURIComponent(T)}&id=eq.${encodeURIComponent(id)}`,data);else await dbInsert(env,'membership_plans',{id,tenant_id:T,created_at:nowIso(),...data});return jsonOk({ok:true,id})}
async function hIssueMembershipPlan(env,b){
  const T=b._tenantId;if(!await verifyStaff(env,b.email,b.token,T,'finance'))return jsonErr('無權限');const planId=String(b.planId||''),plans=await dbGet(env,'membership_plans',`tenant_id=eq.${encodeURIComponent(T)}&id=eq.${encodeURIComponent(planId)}&status=eq.active&select=*`).catch(()=>[]),p=plans[0],ownerEmail=normEmail(b.ownerEmail||'');if(!p||!ownerEmail)return jsonErr('請選擇有效方案與會員');const walletId=genId('WAL'),now=nowIso(),expires=p.valid_days?new Date(Date.now()+safeNum(p.valid_days)*86400000).toISOString():null;
  await dbInsert(env,'customer_wallets',{id:walletId,tenant_id:T,platform_member_id:String(b.platformMemberId||'')||null,owner_email:ownerEmail,shared_group_key:p.sharing_mode==='named_group'?String(b.sharedGroupKey||'').trim()||null:null,account_type:'pass',name:p.name,unit:'times',balance:0,operation_unit_id:p.operation_unit_id,service_item_id:p.service_item_id,status:'active',expires_at:expires,config_json:{planId:p.id,planType:p.plan_type},created_by:b.email||'',created_at:now,updated_at:now});if(safeNum(p.included_quantity)>0)await dbInsert(env,'customer_wallet_ledger',{id:genId('WLE'),tenant_id:T,wallet_id:walletId,registration_id:null,entry_type:'purchase',amount:safeNum(p.included_quantity),note:'方案開通',idempotency_key:String(b.idempotencyKey||genId('ISS')),meta_json:{planId:p.id},created_by:b.email||'',created_at:now});await dbInsert(env,'membership_subscriptions',{id:genId('MSB'),tenant_id:T,plan_id:p.id,wallet_id:walletId,platform_member_id:String(b.platformMemberId||'')||null,owner_email:ownerEmail,status:'active',starts_at:now,expires_at:expires,created_at:now,updated_at:now});return jsonOk({ok:true,walletId,expiresAt:expires});
}
async function hUpdateServiceVisit(env,b){
  const T=b._tenantId,rid=String(b.registrationId||''),status=String(b.status||'');if(!await verifyStaff(env,b.email,b.token,T,'onsite')&&!await verifyStaff(env,b.email,b.token,T,'sessions'))return jsonErr('無權限');if(!['booked','arrived','in_service','completed','no_show','cancelled'].includes(status))return jsonErr('服務狀態不正確');const regs=await dbGet(env,'registrations',`tenant_id=eq.${encodeURIComponent(T)}&id=eq.${encodeURIComponent(rid)}&select=id,operation_unit_id,custom_fields_json`).catch(()=>[]),r=regs[0];if(!r)return jsonErr('找不到預約');const old=await dbGet(env,'service_visits',`tenant_id=eq.${encodeURIComponent(T)}&registration_id=eq.${encodeURIComponent(rid)}&select=*`).catch(()=>[]),now=nowIso(),data={status,updated_by:b.email||'',note:String(b.note||'').slice(0,500),updated_at:now};if(status==='arrived'&&!old[0]?.arrived_at)data.arrived_at=now;if(status==='in_service'&&!old[0]?.started_at)data.started_at=now;if(status==='completed'&&!old[0]?.completed_at)data.completed_at=now;if(old.length)await dbUpdate(env,'service_visits',`tenant_id=eq.${encodeURIComponent(T)}&registration_id=eq.${encodeURIComponent(rid)}`,data);else await dbInsert(env,'service_visits',{id:genId('SVI'),tenant_id:T,registration_id:rid,operation_unit_id:r.operation_unit_id,service_item_id:null,created_at:now,...data});return jsonOk({ok:true,status});
}
async function hSaveWaitlistPolicy(env,b){const T=b._tenantId,sid=String(b.sessionId||''),rows=await dbGet(env,'sessions',`tenant_id=eq.${encodeURIComponent(T)}&id=eq.${encodeURIComponent(sid)}&select=modules_json`).catch(()=>[]);if(!rows.length||!await verifyStaff(env,b.email,b.token,T,'sessions',sid))return jsonErr('無權限');const mods=safeJson(rows[0].modules_json,{});mods.waitlistPolicy={enabled:b.enabled===true||b.enabled==='true',paymentDeadlineHours:Math.max(1,Math.min(168,parseInt(b.paymentDeadlineHours||24,10)||24)),maxPromotionsPerRun:Math.max(1,Math.min(20,parseInt(b.maxPromotionsPerRun||1,10)||1))};await dbUpdate(env,'sessions',`tenant_id=eq.${encodeURIComponent(T)}&id=eq.${encodeURIComponent(sid)}`,{modules_json:JSON.stringify(mods),updated_at:nowIso()});return jsonOk({ok:true,policy:mods.waitlistPolicy})}
async function hSaveMarketingAutomation(env,b){const T=b._tenantId;if(!await verifyStaff(env,b.email,b.token,T,'settings'))return jsonErr('無權限');const name=String(b.name||'').trim(),trigger=['before_booking','after_completion'].includes(String(b.triggerType))?String(b.triggerType):'';if(!name||!trigger||!String(b.titleTemplate||'').trim()||!String(b.bodyTemplate||'').trim())return jsonErr('請填完整的自動通知內容');const id=String(b.id||genId('MKT')),data={operation_unit_id:String(b.operationUnitId||'')||null,name,trigger_type:trigger,offset_minutes:parseInt(b.offsetMinutes||0,10)||0,audience:String(b.audience)==='marketing_consented'?'marketing_consented':'eligible_customers',channel:String(b.channel)==='email'?'email':'system',title_template:String(b.titleTemplate).slice(0,200),body_template:String(b.bodyTemplate).slice(0,2000),active:b.active!==false,config_json:b.config||{},created_by:b.email||'',updated_at:nowIso()},old=await dbGet(env,'marketing_automations',`tenant_id=eq.${encodeURIComponent(T)}&id=eq.${encodeURIComponent(id)}&select=id`).catch(()=>[]);if(old.length)await dbUpdate(env,'marketing_automations',`tenant_id=eq.${encodeURIComponent(T)}&id=eq.${encodeURIComponent(id)}`,data);else await dbInsert(env,'marketing_automations',{id,tenant_id:T,created_at:nowIso(),...data});return jsonOk({ok:true,id})}

async function runWaitlistAutomation(env){
  const sessions=await dbGet(env,'sessions','status=in.(報名中,開放中,open,active)&select=id,tenant_id,limit_count,current_count,modules_json').catch(()=>[]),now=Date.now();
  for(const s of sessions){const policy=safeJson(s.modules_json,{}).waitlistPolicy||{};if(!policy.enabled)continue;const regs=await dbGet(env,'registrations',`tenant_id=eq.${encodeURIComponent(s.tenant_id)}&session_id=eq.${encodeURIComponent(s.id)}&select=*`).catch(()=>[]);for(const r of regs){const snap=safeJson(r.payment_terms_snapshot,{});if(snap.promotedFromWaitlist&&String(r.review_status)==='錄取'&&!isPaidStatus(r.payment_status)&&r.payment_due_at&&new Date(r.payment_due_at).getTime()<=now)await dbUpdate(env,'registrations',`tenant_id=eq.${encodeURIComponent(s.tenant_id)}&id=eq.${encodeURIComponent(r.id)}`,{review_status:'備取',status:'備取',payment_expired_at:nowIso(),updated_at:nowIso()})}
    const fresh=await dbGet(env,'registrations',`tenant_id=eq.${encodeURIComponent(s.tenant_id)}&session_id=eq.${encodeURIComponent(s.id)}&select=*`).catch(()=>[]),used=fresh.filter(r=>['已錄取','錄取','報名成功'].includes(String(r.review_status||r.status||''))).length,slots=Math.max(0,safeNum(s.limit_count)-used),count=Math.min(slots,Math.max(1,safeNum(policy.maxPromotionsPerRun)||1)),wait=fresh.filter(r=>String(r.review_status)==='備取').sort((a,b)=>String(a.created_at).localeCompare(String(b.created_at))).slice(0,count);for(const r of wait){const due=new Date(Date.now()+Math.max(1,safeNum(policy.paymentDeadlineHours)||24)*3600000).toISOString(),snap={...safeJson(r.payment_terms_snapshot,{}),promotedFromWaitlist:true,promotedAt:nowIso(),deadlineHours:policy.paymentDeadlineHours||24};await dbUpdate(env,'registrations',`tenant_id=eq.${encodeURIComponent(s.tenant_id)}&id=eq.${encodeURIComponent(r.id)}`,{review_status:'錄取',status:'錄取',approved_at:nowIso(),payment_due_at:due,payment_terms_snapshot:JSON.stringify(snap),updated_at:nowIso()});await recordNotification(env,{tenantId:s.tenant_id,sessionId:s.id,registrationId:r.id,email:r.email,eventKey:'waitlist_promoted',title:'候補已遞補錄取',body:`請在 ${due.slice(0,16).replace('T',' ')} 前完成後續步驟。`})}}
}
async function runMarketingAutomations(env){
  const rules=await dbGet(env,'marketing_automations','active=eq.true&select=*').catch(()=>[]),now=Date.now();for(const a of rules){const regs=await dbGet(env,'registrations',`tenant_id=eq.${encodeURIComponent(a.tenant_id)}${a.operation_unit_id?'&operation_unit_id=eq.'+encodeURIComponent(a.operation_unit_id):''}&select=*`).catch(()=>[]);for(const r of regs){let due=0;if(a.trigger_type==='after_completion'){const visits=await dbGet(env,'service_visits',`tenant_id=eq.${encodeURIComponent(a.tenant_id)}&registration_id=eq.${encodeURIComponent(r.id)}&status=eq.completed&select=completed_at&limit=1`).catch(()=>[]);if(visits[0])due=new Date(visits[0].completed_at).getTime()+safeNum(a.offset_minutes)*60000}else if(a.trigger_type==='before_booking'&&r.payment_due_at)due=new Date(r.payment_due_at).getTime()-Math.abs(safeNum(a.offset_minutes))*60000;if(!due||due>now||due<now-86400000)continue;if(a.audience==='marketing_consented'){const leads=await dbGet(env,'photo_leads',`tenant_id=eq.${encodeURIComponent(a.tenant_id)}&email=ilike.${encodeURIComponent(r.email||'')}&marketing_consent=eq.true&select=id&limit=1`).catch(()=>[]);if(!leads.length)continue}const scheduled=new Date(due).toISOString(),key=`tenant_id=eq.${encodeURIComponent(a.tenant_id)}&automation_id=eq.${encodeURIComponent(a.id)}&registration_id=eq.${encodeURIComponent(r.id)}&scheduled_for=eq.${encodeURIComponent(scheduled)}&select=id`;if((await dbGet(env,'marketing_automation_runs',key).catch(()=>[])).length)continue;const nid=genId('NTF');await dbInsert(env,'notifications',{id:nid,tenant_id:a.tenant_id,operation_unit_id:r.operation_unit_id||null,session_id:r.session_id||null,registration_id:r.id,member_email:normEmail(r.email),event_key:'automation_'+a.trigger_type,channel:a.channel,title:a.title_template,body:a.body_template,status:'queued',scheduled_at:scheduled,sent_at:null,error_message:null,meta_json:{automationId:a.id},created_at:nowIso()});await dbInsert(env,'marketing_automation_runs',{id:genId('MKR'),tenant_id:a.tenant_id,automation_id:a.id,registration_id:r.id,member_email:normEmail(r.email),scheduled_for:scheduled,status:'created',notification_id:nid,created_at:nowIso()})}}
}
...
const TENANT_PORTABLE_TABLES=Object.freeze([
  ...
  'vendor_sales_reports','consignment_periods','consignment_applications','consignment_products','pos_sales','pos_sale_items','inventory_movements',
  'membership_plans','membership_subscriptions','service_visits','marketing_automations','marketing_automation_runs',
...
async function hLineStart(env,url){
  ...
  const params=new URLSearchParams({response_type:'code',client_id:env.LINE_LOGIN_CHANNEL_ID,redirect_uri:lineRedirectUri(env,url),state,scope,nonce,bot_prompt:'normal',ui_locales:'zh-TW'});
