...
async function ensureOperationUnitEntitlement(env,T,u){
  const m=normalizeSessionModules(safeJson(u.modules_json,{})),mode=String(m.operatingMode||'activity');
  if(mode==='booking'){
    const active=await activeBookingEntitlement(env,T);if(active)return {ok:true,mode,periodEnd:active.period_end};
    const end=addCalendarMonthTaipei(nowIso()),c=await consumeCreditOrNeedPayment(env,T,DOING_BOOKING_MONTHLY_FEE,'booking_monthly','unit:'+u.id,end);if(!c.ok)return {...c,mode};
    try{await dbInsert(env,'billing_logs',{id:genId('BIL'),tenant_id:T,billing_type:'booking_monthly',amount:DOING_BOOKING_MONTHLY_FEE,tax:0,total:DOING_BOOKING_MONTHLY_FEE,status:'confirmed',confirmed_at:nowIso(),confirmed_by:'platform_credit',period_start:nowIso(),period_end:end,note:'預約營運月方案｜'+u.id,created_at:nowIso()})}catch(e){await rollbackPlatformCreditUse(env,T,DOING_BOOKING_MONTHLY_FEE,c.ledgerId,'booking_unit_entitlement_failed').catch(()=>{});throw e}return {ok:true,mode,periodEnd:end};
  }
  if(await hasOperationUnitEntitlement(env,T,u.id))return {ok:true,mode};
  const c=await consumeCreditOrNeedPayment(env,T,DOING_ACTIVITY_PUBLISH_FEE,'activity_unit',u.id);if(!c.ok)return {...c,mode};
  try{await dbInsert(env,'billing_logs',{id:genId('BIL'),tenant_id:T,billing_type:billingTypeForOperationUnit(u.id),amount:DOING_ACTIVITY_PUBLISH_FEE,tax:0,total:DOING_ACTIVITY_PUBLISH_FEE,status:'confirmed',confirmed_at:nowIso(),confirmed_by:'platform_credit',period_start:nowIso(),period_end:null,note:'營運項目正式開通｜'+u.id,created_at:nowIso()})}catch(e){await rollbackPlatformCreditUse(env,T,DOING_ACTIVITY_PUBLISH_FEE,c.ledgerId,'activity_unit_entitlement_failed').catch(()=>{});throw e}return {ok:true,mode};
}
...
async function syncOperationUnitCatalogs(env,T,u){
  ...
  const slots=Array.isArray(pub.timeslots)?pub.timeslots:[];
  ...
  for(let i=0;i<slots.length;i++){
    ...
    const payload={session_id:sid,operation_unit_id:uid,date_key:date,label:String(x.label||date),start_text:start,end_text:end,capacity:Math.max(0,parseInt(x.capacity||x.limit||0,10)||0),status:x.status==='closed'?'closed':'open',updated_at:now};
...
async function hGetOperationUnitsAdmin(env,p){
  ...
  const rows=await dbGet(env,'operation_units',q).catch(()=>[]),out=[];for(const u of rows){const f=formatOperationUnit(u);const ts=await dbGet(env,'timeslots',`tenant_id=eq.${encodeURIComponent(T)}&operation_unit_id=eq.${encodeURIComponent(u.id)}&select=*&order=date_key.asc,start_text.asc`).catch(()=>[]);f.timeslots=ts.map(x=>({id:x.id,date:x.date_key,label:x.label||x.date_key,start:x.start_text||'',end:x.end_text||'',capacity:safeNum(x.capacity),status:x.status||'open'}));out.push(f)}return jsonOk(out);
}
...
async function hApproveApply(env,b){
  ...
  const existing=await findAdminWorkspacesByEmail(env,email);
  if(existing.length)return jsonErr('此 Email 已經有主辦帳號，請勿重複開通');
  ...
  try{
    await dbInsert(env,'tenants',{
      ...
      notify_email:email,config_json:JSON.stringify({moduleProfile:profile}),payment_config_json:'{}',
...
async function issueGoogleOAuthState(env, tenant, extra={}) {
  ...
  return signAdminJwt({
    ...
    mode:String(extra.mode||''), brand_name:String(extra.brand_name||'').slice(0,120), contact_name:String(extra.contact_name||'').slice(0,120), contact_phone:String(extra.contact_phone||'').slice(0,80), event_type:String(extra.event_type||'').slice(0,240), module_profile:String(extra.module_profile||'').slice(0,6000),
    issued_at: now,
    expires_at: now + 10 * 60 * 1000,
  }, env);
}
...
async function hGoogleStart(env, url) {
  ...
  const state = await issueGoogleOAuthState(env, tenant, {mode:url.searchParams.get('mode')||'',brand_name:url.searchParams.get('brand_name')||'',contact_name:url.searchParams.get('contact_name')||'',contact_phone:url.searchParams.get('contact_phone')||'',event_type:url.searchParams.get('event_type')||'',module_profile:url.searchParams.get('module_profile')||''});
...
async function hGoogleCallback(env, url) {
  ...
  if(statePayload.mode==='organizer_signup'){
    const appPayload=safeJson(statePayload.application_payload,{});
    const brand=String(statePayload.brand_name||appPayload.unitName||'').trim();
    const contact=String(statePayload.contact_name||appPayload.contactName||googleName||'').trim();
    const phone=String(statePayload.contact_phone||appPayload.phone||'').trim();
    if(!brand||!contact||!phone)return failRedirect('signup_missing_profile',doingPageUrl(env,'about.html')+'#apply');
    const existingWorkspaces=await findAdminWorkspacesByEmail(env,googleEmail);
    if(existingWorkspaces.length){
      const sel=await issueWorkspaceSelectionToken(googleEmail,existingWorkspaces.map(x=>x.tenant_id),env);
      const u=new URL(loginUrl);u.searchParams.set('workspace_token',sel);return Response.redirect(u.toString(),302);
    }
    let signupProfile=safeJson(statePayload.module_profile,{});
    ...
    const applicationJson={...appPayload,moduleProfile:profile,googleName,googleSub:String(userInfo.sub||'')};
    const existingRows=await dbGet(env,'tenant_apply_logs',`contact_email=eq.${encodeURIComponent(googleEmail)}&status=in.(pending,supplement_required)&select=id,status,supplement_count`).catch(()=>[]);
    const pending=existingRows.find(x=>String(x.status)==='pending');
    const supplement=existingRows.find(x=>String(x.status)==='supplement_required');
    if(pending){
      const u=new URL(doingPageUrl(env,'about.html'));u.hash='apply';u.searchParams.set('application_status','pending');
      return Response.redirect(u.toString(),302);
    }
    if(supplement){
      await dbUpdate(env,'tenant_apply_logs',`id=eq.${encodeURIComponent(supplement.id)}`,
        {
          brand_name:brand,contact_name:contact,contact_phone:phone,event_type:String(statePayload.event_type||''),status:'pending',
      ...
      const u=new URL(doingPageUrl(env,'about.html'));u.hash='apply';u.searchParams.set('application_status','supplement_submitted');
      return Response.redirect(u.toString(),302);
    }
    await dbInsert(env,'tenant_apply_logs',{
      id:genId('APL'),brand_name:brand,contact_name:contact,contact_email:googleEmail,contact_phone:phone,
      event_type:String(statePayload.event_type||''),plan_type:'review',note:'Google 驗證完成',
      status:'pending',application_json:applicationJson,created_at:nowIso()
    });
    const u=new URL(doingPageUrl(env,'about.html'));u.hash='apply';u.searchParams.set('application_status','pending');
    return Response.redirect(u.toString(),302);
  }
...
async function prepareRegistration(env, b) {
  ...
  if(modules.workshopSlots){
    ...
    const slots=await dbGet(env,'timeslots',`tenant_id=eq.${TENANT}&session_id=eq.${encodeURIComponent(b.sessionId)}${unitSlotFilter}&id=eq.${encodeURIComponent(claimedTimeslotIds[0])}&status=eq.open&select=id`);
  ...
  const row = {
    ...
    session_id:b.sessionId, operation_unit_id:operationUnit?operationUnit.id:null, event_id:cleanEventId(ses.event_id),
...
const DOING_ACTIVITY_PUBLISH_FEE=200,DOING_BOOKING_MONTHLY_FEE=888;
...
async function hConfirmOperatingPayment(env,b){
  const pay=await verifyAdminJwt(b.token,env);if(!pay||pay.normalized_role!=='platform_super_admin')return jsonErr('無權限');
  const T=String(b.target_tenant_id||'').trim().toLowerCase(),mode=String(b.mode||'').trim(),sid=String(b.sessionId||'').trim();if(!T)return jsonErr('請選擇主辦');
  if(mode==='booking'){
    ...
    const start=nowIso(),end=addCalendarMonthTaipei(start);await dbInsert(env,'billing_logs',{id:genId('BIL'),tenant_id:T,billing_type:'booking_monthly',amount:DOING_BOOKING_MONTHLY_FEE,tax:0,total:DOING_BOOKING_MONTHLY_FEE,status:'confirmed',confirmed_at:start,confirmed_by:pay.email,period_start:start,period_end:end,note:String(b.note||'平台確認預約營運款'),created_at:start});const pendingUnits=await dbGet(env,'operation_units',`tenant_id=eq.${encodeURIComponent(T)}&status=eq.pending_payment&select=id,modules_json`).catch(()=>[]);for(const u of pendingUnits){if(String(normalizeSessionModules(safeJson(u.modules_json,{})).operatingMode||'activity')==='booking')await dbUpdate(env,'operation_units',`tenant_id=eq.${encodeURIComponent(T)}&id=eq.${encodeURIComponent(u.id)}`,{status:'open',updated_at:start}).catch(()=>{})}return jsonOk({ok:true,mode,amount:DOING_BOOKING_MONTHLY_FEE,periodEnd:end});
  }
  if(mode==='operation_unit'){
    const uid=String(b.operationUnitId||'').trim();if(!uid)return jsonErr('請指定營運項目');const ur=await dbGet(env,'operation_units',`tenant_id=eq.${encodeURIComponent(T)}&id=eq.${encodeURIComponent(uid)}&select=id,modules_json`).catch(()=>[]);if(!ur.length)return jsonErr('找不到該主辦的營運項目');const um=normalizeSessionModules(safeJson(ur[0].modules_json,{}));if(String(um.operatingMode||'activity')==='booking')return jsonErr('此營運項目屬預約月方案，請開通預約營運權');if(await hasOperationUnitEntitlement(env,T,uid))return jsonOk({ok:true,alreadyActive:true});const t=nowIso();await dbInsert(env,'billing_logs',{id:genId('BIL'),tenant_id:T,billing_type:billingTypeForOperationUnit(uid),amount:DOING_ACTIVITY_PUBLISH_FEE,tax:0,total:DOING_ACTIVITY_PUBLISH_FEE,status:'confirmed',confirmed_at:t,confirmed_by:pay.email,period_start:t,period_end:null,note:String(b.note||'平台確認營運項目開通款'),created_at:t});await dbUpdate(env,'operation_units',`tenant_id=eq.${encodeURIComponent(T)}&id=eq.${encodeURIComponent(uid)}&status=eq.pending_payment`,{status:'open',updated_at:t}).catch(()=>{});return jsonOk({ok:true,mode,amount:DOING_ACTIVITY_PUBLISH_FEE,operationUnitId:uid});
  }
  if(mode!=='activity'||!sid)return jsonErr('活動發布請指定場次');const sr=await dbGet(env,'sessions',`tenant_id=eq.${encodeURIComponent(T)}&id=eq.${encodeURIComponent(sid)}&select=id`).catch(()=>[]);if(!sr.length)return jsonErr('找不到該主辦的場次');if(await hasActivityEntitlement(env,T,sid))return jsonOk({ok:true,alreadyActive:true});const t=nowIso();await dbInsert(env,'billing_logs',{id:genId('BIL'),tenant_id:T,billing_type:billingTypeForActivity(sid),amount:DOING_ACTIVITY_PUBLISH_FEE,tax:0,total:DOING_ACTIVITY_PUBLISH_FEE,status:'confirmed',confirmed_at:t,confirmed_by:pay.email,period_start:t,period_end:null,note:String(b.note||'平台確認活動發布款'),created_at:t});return jsonOk({ok:true,mode,amount:DOING_ACTIVITY_PUBLISH_FEE,sessionId:sid});
}
...
async function ensureOperatingEntitlement(env,T,s){
  const mods=normalizeSessionModules(safeJson(s.modules_json,{}));const mode=String(mods.operatingMode||'activity');
  if(mode==='booking'){
    const act=await activeBookingEntitlement(env,T);if(act)return {ok:true,mode,periodEnd:act.period_end};
    const end=addCalendarMonthTaipei(nowIso()),c=await consumeCreditOrNeedPayment(env,T,DOING_BOOKING_MONTHLY_FEE,'booking_monthly',s.id,end);if(!c.ok)return {...c,mode};
    const raced=await activeBookingEntitlement(env,T);if(raced){await rollbackPlatformCreditUse(env,T,DOING_BOOKING_MONTHLY_FEE,c.ledgerId,'booking_entitlement_already_created').catch(()=>{});return {ok:true,mode,periodEnd:raced.period_end}}
    try{
      await dbInsert(env,'billing_logs',{id:genId('BIL'),tenant_id:T,billing_type:'booking_monthly',amount:DOING_BOOKING_MONTHLY_FEE,tax:0,total:DOING_BOOKING_MONTHLY_FEE,status:'confirmed',confirmed_at:nowIso(),confirmed_by:'platform_credit',period_start:nowIso(),period_end:end,note:'預約營運月方案',created_at:nowIso()});
    }catch(e){await rollbackPlatformCreditUse(env,T,DOING_BOOKING_MONTHLY_FEE,c.ledgerId,'booking_entitlement_failed').catch(()=>{});throw e}
    return {ok:true,mode,periodEnd:end};
  }
  if(await hasActivityEntitlement(env,T,s.id))return {ok:true,mode};
  const c=await consumeCreditOrNeedPayment(env,T,DOING_ACTIVITY_PUBLISH_FEE,'activity_publish',s.id);if(!c.ok)return {...c,mode};
  if(await hasActivityEntitlement(env,T,s.id)){await rollbackPlatformCreditUse(env,T,DOING_ACTIVITY_PUBLISH_FEE,c.ledgerId,'activity_entitlement_already_created').catch(()=>{});return {ok:true,mode}}
  try{
    await dbInsert(env,'billing_logs',{id:genId('BIL'),tenant_id:T,billing_type:billingTypeForActivity(s.id),amount:DOING_ACTIVITY_PUBLISH_FEE,tax:0,total:DOING_ACTIVITY_PUBLISH_FEE,status:'confirmed',confirmed_at:nowIso(),confirmed_by:'platform_credit',period_start:nowIso(),period_end:null,note:'活動發布權',created_at:nowIso()});
  }catch(e){await rollbackPlatformCreditUse(env,T,DOING_ACTIVITY_PUBLISH_FEE,c.ledgerId,'activity_entitlement_failed').catch(()=>{});throw e}
  return {ok:true,mode};
}
...
async function hRescheduleBooking(env,b){
  ...
  try{
    ...
    await dbUpdate(env,'registrations',`tenant_id=eq.${T}&id=eq.${encodeURIComponent(reg.id)}`,{selected_dates_json:JSON.stringify([newRows[0].date_key]),custom_fields_json:JSON.stringify(cf),transfer_balance_due:nextBalance,admin_note:(String(reg.admin_note||'')+` [系統] 改期 ${count+1} 次：${oldIds.join(',')} → ${newId} ${nowTaipeiText()}`).trim()});
