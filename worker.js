Y��x-���jם��i��+��j[h��ܢ���Ot�|ﾴo+^����ם// DOING_MUST_FIX_CLOSURE_20260725：付款回報撤回／主辦全流程取消／不可抗力延期完整資料與金流搬移／SaaS feature gate 保留
...
async function hPlatformUpsertTenantOwner(env,b){
  ...
  const data={
    ...
    role_id:null,
    perms_json:JSON.stringify({events:true,sessions:true,review:tr�my��$z{-���jם=[b.lineNotified?'LINE 已通知':'',b.phoneContacted?'電話已聯絡':''].filter(Boolean).join('｜');data.admin_note=appendNote(label+(mode==='lateFlag'&&mins?(' '+mins+' 分鐘'):'')+(contact?'｜'+contact:''));
