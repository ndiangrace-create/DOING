  ...
  if(!document.querySelector('.d2-topbar')){
    ...
    const sync=()=>{
      ...
      if(desktop){
        ...
        top.style.setProperty('z-index','1000','important');
        requestAnimationFrame(()=>document.body.style.setProperty('padding-top',top.offsetHeight+'px','important'));
...
if(product==='market'){
  ...
  if(settings){
    settings.innerHTML=[['付款資訊','銀行／既有付款方式／收款設定','payment'],['合約管理','報名前同意條款與版本','agreement'],['信件／通知模板','錄取／補件／付款／行前通知','notification'],['工作人員／權限','主辦／財務／現場與場次權限','team'],['公司／品牌資料','營運名稱、聯絡與品牌資訊','brand'],['首頁顯示','公告、封面、LINE 與前台顯示','front'],['主題活動','系列／活動主題與首頁精選','theme'],['常用場地圖','保存、重用場地圖與攤位','map'],['活動拍照框','活動框、行銷與名單','photo'],['現場通行碼','現場人員快速操作入口','onsite'],['系統異常／稽核','錯誤紀錄與查修依據','audit']].map(([a,b,key])=>`<a href="#settings" data-setting-key="${key}">${a}<small>${b}</small></a>`).join('');
  }
}
